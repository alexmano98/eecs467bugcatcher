...
bool test_saved_poses(const std::string& mapFile, const std::string& posesFile, const std::string& testName)
{
    ...
    for(int n = 0; n < numGoals; ++n)
    {
        ...
        if(foundPath)
        {
            ...
            // size_t lastindex = mapFile.find_last_of("."); 
            // std::string outPathFile = mapFile.substr(0, lastindex) + "_path_"  + std::to_string(n) + ".txt";
            // std::ofstream posOut(outPathFile);
            // posOut <<  start.x << " "  << start.y << " "  << goal.x << " "  << goal.y << " "  << shouldExist << "\n";

            // for(int i = 0; i < path.path_length; i++)
            // {
            //     auto p = path.path[i];
            //     posOut << p.utime << " " << p.x << " " << p.y << " " << p.theta << "\n";
            // }
            // posOut.close();
            // std::cout << "Wrote to file: " << outPathFile << std::endl;
        }
        else
        {
            if(shouldExist)
