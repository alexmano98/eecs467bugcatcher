...
// Alex's code
// Iterate through path and remove points that are within tbe robot's radius of another point
void remove_close_poses(robot_path_t& currentPath){
    
    // // Iterate through the current path and get the distance from each node to the next
    for(int i = 1; i < currentPath.path_length; i++){

        // Calc dist from path[i] to path[i-1]
        pose_xyt_t curr_pose = currentPath.path[i];
        pose_xyt_t prev_pose = currentPath.path[i - 1];
        double deltax = curr_pose.x - prev_pose.x;
        double deltay = curr_pose.y - prev_pose.y;
        double dist = sqrt(deltax * deltax + deltay * deltay);

        // If distance is less than MBOT_RADIUS, remove path[i] and decrement i and path_length
        if(dist < MBOT_RADIUS && currentPath.path_length > 2){
            currentPath.path.erase(currentPath.path.begin() + i);
            i--;
            currentPath.path_length--;
        }
    }
}
...
{
    //////////////////////// DONE: Implement your method for exploring the map ///////////////////////////
    /*
    * NOTES:
    ...
    */
    ...
    // set the obstacle distance grid map in the motion planner
    planner_.setMap(currentMap_);

    // finds all frontiers in the map
    frontiers_ =  find_map_frontiers(currentMap_, currentPose_);
    std::cout << "size of frontier " << frontiers_.size() << std::endl;
    // select a frontier to explore and returns a path
    // TODO: check if the planned path is a valid path
    // std::cout << "current pose at " << currentPose_.x << ' ' << currentPose_.y << std::endl;


    // TODO: Add flag to make sure we have moved a couple waypoints
    if(!currentPath_.path.empty() && !frontiers_.empty()) {
        if ( sqrt((currentPose_.x - currentPath_.path[0].x) * (currentPose_.x - currentPath_.path[0].x) + 
            (currentPose_.y - currentPath_.path[0].y) * (currentPose_.y - currentPath_.path[0].y)) 
             > MBOT_RADIUS / 2)
        {
            currentPath_ = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
            planner_.setPrevGoal(currentPath_.path.back());
            remove_close_poses(currentPath_);
        }
    }
    ...
    if (currentPath_.path_length > 1) {
        std::cout << "found path from " << currentPose_.x << ' ' << currentPose_.y << '\n';
        for(auto pose : currentPath_.path){
            std::cout << "(" << pose.x << "," << pose.y << "," << pose.theta << "); ";
        }std::cout << "\n";
    }
    ...
    else
    {
        ...
        std::cout << "failed to find path here \n";
    }
...
{
    //////////////////////// DONE: Implement your method for returning to the home pose ///////////////////////////
    /*
    * NOTES:
    *   - At the end of each iteration, then (1) or (2) must hold, otherwise exploration is considered to have failed:
    ...
    */
    ...
    // std::cout << "current pose at " << currentPose_.x << ' ' << currentPose_.y << std::endl;
    // currentPath_ = planner_.planPath(currentPose_, homePose_);
    // std::cout << "home path length " << currentPath_.path.size() << std::endl;
