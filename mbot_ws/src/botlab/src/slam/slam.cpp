...
void OccupancyGridSLAM::runSLAM(void)
{
    while(true)
    {
        // std::cout << "trying to run slam program\n";
        // If new data has arrived
        if(isReadyToUpdate())
        {
            // std::cout << "reading to run slam \n";
...
void OccupancyGridSLAM::runSLAMIteration(void)
{
    ...
    // std::cout << "current laser scan range: " << currentScan_.num_ranges << std::endl;
