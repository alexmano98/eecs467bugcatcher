...
int BotGui::onKeyEvent(vx_layer_t* layer, vx_key_event_t* event)
{
    ...
    if (event->key_code == VX_KEY_ENTER) {
        std::ofstream fs;
        fs.open ("trueposes.csv");        
        fs << "Trueposes\ntime,x,y,theta\n";
        for (PoseTrace::const_iterator it = traces_[TRUE_POSE_CHANNEL].trace.begin(); 
        it != traces_[TRUE_POSE_CHANNEL].trace.end(); ++it) {
            fs << it->utime << ',' << it->x << ',' << it->y << ',' << it->theta << '\n';
        }   
        fs.close(); 
        fs.open ("slamposes.csv"); 
        fs << "SLAMposes\ntime,x,y,theta\n";
        for (PoseTrace::const_iterator it = traces_[SLAM_POSE_CHANNEL].trace.begin(); 
        it != traces_[SLAM_POSE_CHANNEL].trace.end(); ++it) {
            fs << it->utime << ',' << it->x << ',' << it->y << ',' << it->theta << '\n';
        }  
        fs.close();
    }
...
void BotGui::handlePose(const bot_msgs::pose_xyt_t::ConstPtr& pose)
{
    ...
    //     truePoses_.push_back(*pose); // stores all true poses
    // }
}
